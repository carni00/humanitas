...
(*  let pl   = Nil.map  Partitio.primary pnl in*)
  ...
  let n_nl = NatioList.update gnl cl nl rl luc pl pal in
...
let create forme size =
  ...
  let nl   = NatioList.update gnl cl nl rl luc pl (Nil.init (fun i -> []) nil) in

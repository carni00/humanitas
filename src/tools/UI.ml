...
module Make(B : Backend) = struct
  ...
  and frame = {
    ...
    fr_focus_request : [`focus | `unfocus] event ;
  }
  and label = {
  ...
  let overlay ?(w = c `tight) ?(h = c `tight) ?(state = c `active) children =
  ...
  (* let rec focus_requests widget = match widget.kind with *)
  (*   | Frame fr ->  *)
  (*     E.merge (fun l x -> x @ l) [] [  *)
  (* 	E.map (fun x -> [ x, widget.id ]) fr.fr_focus_request ;  *)
  (* 	focus_requests fr.fr_child  *)
  (*     ] *)
	
  (*   | HPack { hp_children = children } *)
  (*   | VPack { vp_children = children } *)
  (*   | Overlay children -> *)
  (*     E.merge (fun l x -> x @ l) [] (List.map focus_requests children) *)
	
  (*   | WSignal ws -> bind_e ws focus_requests *)

  (*   | _ -> E.never *)


  (* let focus_signal initial_focus widget = *)
  (*   let init = match initial_focus with *)
  (*     | None -> None *)
  (*     | Some w -> Some w.id in *)
  (*   let update old req = match old, req with *)
  (*     | _, (`focus, id) -> Some id *)
  (*     | Some old_id, (`unfocus, id) when old_id = id -> None *)
  (*     | _ -> old *)
  (*   in *)

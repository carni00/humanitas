...
module Make(B : Backend) : sig
  ...
  val hpack : 
    ?w:size_spec signal -> ?h:size_spec signal -> 
    ?state:state signal ->
    ?layout:hcentering -> ?align:valign ->
    widget list -> widget

  val vpack : 
    ?w:size_spec signal -> ?h:size_spec signal -> 
    ?state:state signal ->
    ?layout:vcentering -> ?align:halign ->
    widget list -> widget
  ...
  val stateful :
    ('a signal -> widget * ('a -> 'a) event) -> 'a -> widget * 'a signal

  val map_s :
    ('a -> widget * 'b) -> 'a signal -> widget * 'b signal

  val map_s_e :
    ('a -> widget * 'b event) -> 'a signal -> widget * 'b event

  val map_e :
    ('a -> widget * 'b) -> 'a -> 'a event -> widget * 'b * 'b event

  (* val loop_s : *)
  (*   ('a -> widget * 'a signal) -> 'a -> widget * 'a signal *)

  val loop_e :
    ('a -> widget * 'a event) -> 'a -> widget * 'a event

...
(* module UI = UI.Make(Irrlicht_backend) *)
...
(*     let panel n =  *)
(*       let buttons, fires =  *)
(* 	List.init n (fun _ -> button (sprintf "Click me! : %d" n) |> Tuple3.get13) *)
(*         |> List.split *)
(*       in *)
(*       vpack buttons,  *)
(*       E.stamp (E.select fires) (succ n) *)
(*     in *)
(*     fst (loop_e panel 1) *)

(*   let ui = UI.make topwidget *)
(* end *)

(* let run () = *)
(*   let device = Irrlicht_backend.device in *)
(*   let driver = device#driver and smgr = device#scene_manager in *)
(*   device#set_window_caption "Humanitas"; *)
(*   let env = device#gui_env in *)
(*   let _ = env#add_window ~text:"Test window" (100, 100, 300, 200) in *)
...
(*   mesh_node#set_material_flag `lighting true; *)
(*   let camera = smgr#add_camera () in *)
(*   camera#set_pos (-20., 150., -20.); *)
(*   camera#set_target (200., -80., 150.); *)
(*   camera#set_far_value (20000.); *)

(*   Irrlicht_backend.send_event := ( *)
(*     function *)
(*       | `mouse ms -> UI.mouse_update ms *)
(*   ) ; *)

...
let arrayCreate e (ll, dl, rl, sl) =
  ...
  let vm = Tmap.init (fun _ _ -> 0) hl rl in (*variation des lat, évoluera à chaque x*)
  ...
  let f x vm =
    ...
    let g y =
      let rec f = function
      | [] -> 0 (*EQUAT*)
      | lat::_ when (y < asy vm ED.nord (Tlist.following lat rvll)
    ...
    let alea _ s v iv sv = match C.situ (abs v) (w-x) with
...
(*  let pluvDList= [    0;  120;  240;   360;   480] in (* surplus de precipitations respectives sur un désert *)*)
  ...
  let f rid = 
    let _,lon = E.Cylinder.yx_of_rrid (E.resolution e) rid in

  ...
  let rec flumenListCreate fleuveID flumenList =  
    if fleuveID >= mw lsl 8 then flumenList
      ...
      then
        ...
        let rewriteWa flumen = Tlist.iteri (fun _ regio ->(Ria.set wa regio.F.rid (flumen.F.fid))) flumen.F.cours in
        let flumenList = 
          if flumen<>F.null 
          then match flumen.F.fin with 
          | F.Estuaire _    -> rewriteWa flumen ; flumen::flumenList
          | F.Confluent _ -> rewriteWa flumen ; insert flumenList flumen
